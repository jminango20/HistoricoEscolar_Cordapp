...
object EnviarHistoricoFlow {
    ...
    //Recebemos o ID do Histórico Escolar que queremos enviar, e qual é o outro node, faculdade, que irá receber o Histórico.
    @InitiatingFlow
    class ReqFlow(val historicoId: UUID, val para: Party): FlowLogic<SignedTransaction>(){
        @Suspendable
        override fun call(): SignedTransaction {

        //1) Consultar na DB o State atual deste Histórico Escolar. Para isso, utilizamos o serviço de “Vault”,
        // que irá manter o estado final.
            //Queremos consultar um Histórico escolar que tenha o linearId igual ao que foi recebido por parametro.
            ...
            val historicoStateAndRef = serviceHub.vaultService.queryBy<HistoricoEscolarState>(QueryCriteria.LinearStateQueryCriteria(linearId = listOf(
                UniqueIdentifier(id = historicoId)))).states.single()
            //Por enquanto so precisamos do State
            val historicoState = historicoStateAndRef.state.data

        //2) Validar para ter certeza de que não estamos transmitindo informação de outra faculdade.
            requireThat {
                "Eu tenho que ser a faculdade emissora do historico para enviar para outra faculdade" using (historicoState.historicoEscolar.faculdade == ourIdentity)
            }
        ...
        //4) Criar o novo estado do State
            ...
            val novoHistoricoState = historicoState.copy(faculdadesReceptoras = historicoState.faculdadesReceptoras + para)

        //5) Criamos o comando
            val comando = Command(HistoricoEscolarContract.Commands.EnviarHistoricoEscolar(), novoHistoricoState.participants.map { it.owningKey })
         ...
         //Responder à chamada de coleta de assinatura, com o “SignTransactionFlow”
            val signTransactionFlow = object : SignTransactionFlow(session){
                override fun checkTransaction(stx: SignedTransaction) = requireThat{
                    ...
                    "As disciplinas nao podem ser emitidas no meu nome." using outputs.all { it.historicoEscolar.faculdade != ourIdentity }
                }
